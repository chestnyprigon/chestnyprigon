    ...
    <main className="catalog-page">
      ...
      <section className="catalog-workspace" id="catalog-list">
        ...
        <aside className={filtersOpen ? "catalog-filters is-open" : "catalog-filters"}>
          ...
          <label className="filter-select"><span>Модель</span><select value={model} disabled={brand === "Все"} onChange={(event) => { setModel(event.target.value); setTrim("Все"); setTrimOptions([]); }}><option value="Все">{brand === "Все" ? "Сначала выберите марку" : "Все модели"}</option>{models.filter((item) => item !== "Все").map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
