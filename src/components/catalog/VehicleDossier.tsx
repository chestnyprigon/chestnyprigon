...
function InspectionEvidence({ images, hasAccident }: { images: InspectionSummary["inspectionImages"]; hasAccident: boolean }) {
  ...
  return <div className="dossier-inspection-evidence">{images.map((image) => <figure key={image.url}><div><Image src={image.url} alt={`${image.title} — официальный осмотр Encar`} fill unoptimized={image.url.startsWith("https://ci.encar.com/")} sizes="(max-width: 700px) 100vw, 380px" /></div><figcaption><span>{image.title}</span><small>Официальное изображение осмотра Encar</small></figcaption></figure>)}<p className={hasAccident ? "is-alert" : ""}><Check size={13} />{hasAccident ? "В истории есть отметки — требуется ручная проверка" : "В отчёте Encar критических отметок кузова нет"}</p></div>;
}
...
function ClientPriceTable({
  ...
  return <details className="dossier-calculation">
    ...
    <div className="dossier-client-table">
      <button className="dossier-price-refresh" type="button" onClick={onRefresh} disabled={isRefreshing}>
        <RefreshCw size={14} className={isRefreshing ? "is-spinning" : ""} />
        {isRefreshing ? "Обновляем цену…" : "Обновить цену"}
      </button>
      {refreshError ? <p className="dossier-price-refresh-error">{refreshError}</p> : null}
      <p><span>Цена авто (KRW)</span><b>{krw(calculation.sourcePriceKrw)}</b></p>
      <p><span>Рабочий курс KRW/USD</span><b>₩{number.format(calculation.krwPerUsd)}</b></p>
...
export function VehicleDossier({ car, pricingContext, catalogHref = "/catalog" }: { car: CatalogCar; pricingContext: PricingContext; catalogHref?: string }) {
  ...
  const [liveCalculation, setLiveCalculation] = useState<BelarusPriceCalculation | null>(null);
  const [liveKrwUsdRate, setLiveKrwUsdRate] = useState<KrwUsdRate | null>(null);
  ...
  const changePreferential = (value: boolean) => {
    setPreferential(value);
    setLiveCalculation(null);
    setLiveKrwUsdRate(null);
  ...
  const refreshPrice = async () => {
    ...
    setPriceRefreshError(null);
    try {
      const response = await fetch(`/api/catalog/${car.id}/price`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ preferential }),
      });
      const payload = await response.json() as { calculation?: BelarusPriceCalculation; krwUsdRate?: KrwUsdRate; error?: string };
      if (!response.ok || !payload.calculation || !payload.krwUsdRate) {
        throw new Error(payload.error ?? "Не удалось обновить цену.");
      }
      setLiveCalculation(payload.calculation);
      setLiveKrwUsdRate(payload.krwUsdRate);
    } catch (error) {
      setPriceRefreshError(error instanceof Error ? error.message : "Не удалось обновить цену.");
    } finally {
  ...
  return <main className="dossier-page">
    <header className="catalog-header">
      ...
      <nav className={menuOpen ? "catalog-nav is-open" : "catalog-nav"}>
        ...
        <Link href={`/?vehicleId=${encodeURIComponent(car.id)}#contacts`} onClick={() => setMenuOpen(false)}>Контакты</Link>
      </nav>
      <Link className="premium-header-cta" href={`/?vehicleId=${encodeURIComponent(car.id)}#contacts`}>Получить консультацию <ArrowRight size={16} /></Link>
    ...
    {leadOpen ? <div className="consult-modal-backdrop" onMouseDown={() => setLeadOpen(false)}><div className="consult-modal vehicle-lead-modal" role="dialog" aria-modal="true" aria-label="Заявка на автомобиль" onMouseDown={(event) => event.stopPropagation()}><button className="consult-close" type="button" onClick={() => setLeadOpen(false)} aria-label="Закрыть"><X /></button>{leadSent ? <div className="lead-success"><Check /><b>Заявка принята</b><span>Менеджер свяжется с вами по этому автомобилю.</span></div> : <><header className="vehicle-lead-heading"><p className="premium-kicker"><span />Заявка на автомобиль</p><h2>{car.brand} {car.model}</h2><p>{car.year} · {car.trim}</p><strong>{formatUsdWithByn(calculation.totalUsd, calculation.rates)} <small>предварительно под ключ</small></strong></header><form className="vehicle-lead-form" onSubmit={submitLead}><div className="vehicle-lead-grid"><label><span>Ваше имя</span><input name="name" placeholder="Например, Евгений" required /></label><label><span>Телефон</span><input name="phone" placeholder="+375 (__) ___-__-__" inputMode="tel" required /></label></div><label><span>Комментарий <i>необязательно</i></span><textarea name="message" placeholder="Напишите, что важно: срок, бюджет или вопросы по автомобилю" rows={4} /></label><footer><small>Отправляя заявку, вы соглашаетесь с обработкой персональных данных.</small><button type="submit" disabled={leadSending}>{leadSending ? "Отправляем…" : "Оставить заявку"} <ArrowRight /></button></footer>{leadError ? <p className="vehicle-lead-error" role="alert">{leadError}</p> : null}</form></>}</div></div> : null}
  </main>;
}
