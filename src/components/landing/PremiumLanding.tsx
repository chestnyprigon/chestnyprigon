...
const processSteps = [
  ...
  { title: "Подбор", short: "Отбираем автомобили под ваши критерии", icon: Search, text: "Сравниваем предложения Encar по состоянию, комплектации, пробегу и цене.", points: ["Только актуальные объявления", "Без лизинга, аренды и такси", "Сравнение сильных вариантов"] },
...
const faqs = [
  ...
  ["Какие автомобили не попадают в каталог?", "Мы исключаем лизинг, долгосрочную аренду, такси, коммерческое использование и объявления с недостаточными или противоречивыми данными."],
...
export function PremiumLanding() {
  ...
  return <main>
    ...
    <section className="premium-catalog premium-section" id="catalog"><div className="premium-section-head"><div><p className="premium-kicker"><span />Каталог Кореи</p><h2>Свежие предложения<br />с понятной историей</h2></div><div className="catalog-meta"><span className="korea-pill">🇰🇷 Только Корея</span><p>Демонстрационные карточки до подключения Encar</p></div></div><div className="premium-car-grid">{cars.map((car, index) => <article className="premium-car-card" key={car.name}><div className="premium-car-image"><Image src={car.image} alt="" fill sizes="(max-width: 720px) 100vw, 33vw" style={{ objectFit: "cover", objectPosition: `${62 + index * 7}% center` }} /><span>{car.status}</span><small>КОРЕЯ 🇰🇷</small></div><div className="premium-car-body"><h3>{car.name}</h3><p>{car.year} · {car.mileage}<br />{car.specs}</p><div><strong>{car.price}</strong><button type="button" onClick={() => setModalOpen(true)}>Подробнее <ArrowRight size={15} /></button></div></div></article>)}</div><div className="premium-catalog-footer"><p><ShieldCheck size={18} />Лизинг, аренда, такси и коммерческие автомобили исключаются до публикации.</p><button className="premium-button primary" type="button" onClick={() => setModalOpen(true)}>Подобрать авто <ArrowRight size={17} /></button></div></section>

    <section className="premium-about premium-section" id="about"><div className="about-visual"><div className="about-orbit"><span>СЕУЛ</span><b>KR</b><small>Минск</small></div></div><div className="about-copy"><p className="premium-kicker"><span />О компании</p><h2>Спокойная сделка.<br />Прозрачный результат.</h2><p>Мы соединяем локальную команду в Беларуси с партнёрами в Южной Корее. Проверяем данные до покупки и берём на себя сложные этапы сделки.</p><div className="about-benefits"><div><ShieldCheck /><span><b>Прозрачность</b><small>Полная структура цены и статусы сделки</small></span></div><div><Clock3 /><span><b>Оперативность</b><small>Подбор и обратная связь без задержек</small></span></div><div><BadgeCheck /><span><b>Надёжность</b><small>Проверяем историю и документы</small></span></div><div><Headphones /><span><b>Поддержка</b><small>Остаёмся на связи до выдачи</small></span></div></div></div></section>
    ...
    <footer className="premium-footer"><div className="footer-brand"><b>ЧЕСТНЫЙ<br /><em>ПРИГОН</em></b><p>Автомобили из Кореи<br />с доставкой в Беларусь</p></div><div><b>Услуги</b><a href="#catalog">Подбор автомобиля</a><a href="#services">Проверка и покупка</a><a href="#services">Доставка</a><a href="#services">Таможенное оформление</a></div><div><b>Контакты</b><a href="tel:+375447543987">+375 (44) 754-39-87</a><a href="mailto:Chestnyjprigon@gmail.com">Chestnyjprigon@gmail.com</a><span>Минск, ул. Мележа, 3</span></div><div><b>Мы в соцсетях</b><div className="footer-social"><button type="button" onClick={() => setModalOpen(true)}><Send /></button><button type="button" onClick={() => setModalOpen(true)}><Camera /></button><button type="button" onClick={() => setModalOpen(true)}><Video /></button></div></div></footer>
