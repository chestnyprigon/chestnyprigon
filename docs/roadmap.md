...
| 5 | Filter ineligible/problematic listings | Enrichment checkpoint complete, expansion pending | Versioned screening excludes confirmed rental, taxi, commercial and problematic listings; missing reports remain public as “Нет данных” |
...
This prevents confirmed rental, taxi, commercial and technically invalid listings from appearing even temporarily. Missing reports are not an exclusion and are shown as “Нет данных”; accident history is not an exclusion and is published together with the available Encar evidence. The first parser run must be a limited verification batch; full initial loading begins only after screening precision and update/removal behavior are checked.
...
- Publications are explicit and the database trigger refuses rows that did not pass screening. Canonical Encar report enrichment is used when available; missing reports do not block an otherwise eligible listing and are marked “Нет данных”.
